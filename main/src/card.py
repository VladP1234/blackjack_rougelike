...
class Card:
    def __init__(self, value, image_path, pos, name = None, on_reveal_effect: Effect|None = None, on_blackjack_effect: Effect|None = None) -> None:
        self.value = value
        self.name = name
    ...
    # Convert the card to a JSON-serializable dictionary
        return {
            ...
            'on_reveal_effect': self.on_reveal_effect_serialise.effect_type.__name__ if self.on_reveal_effect else None,
            'on_reveal_effect_attrs': self.on_reveal_effect_serialise.params if self.on_reveal_effect else None,
            'on_reveal_effect_target': self.on_reveal_effect_serialise.target if self.on_reveal_effect else None,
            'on_blackjack_effect': self.on_blackjack_effect_serialise.effect_type.__name__ if self.on_blackjack_effect else None,
            'on_blackjack_effect_attrs': self.on_blackjack_effect_serialise.params if self.on_blackjack_effect else None,
            'on_blackjack_effect_target': self.on_blackjack_effect_serialise.target if self.on_blackjack_effect else None,
        }
    @staticmethod
