...
class MerchantManager:
    def __init__(self, UIManager: pygame_gui.UIManager, player: Player) -> None:
        ...
        self.ui = []
        self.leave_merchant_button = pygame_gui.elements.UIButton(pygame.Rect(500, 525, 200, 50), text="Back to Map", manager=UIManager, tool_tip_text="Once you leave, you can't return")
    ...
    def draw(self, screen: pygame.Surface):
        for button, card in self.cards.items():
            card.draw(screen, button.relative_rect.left + 10, -5-button.relative_rect.top)
        if self.warning_message:
            # Calculate the position for the text to be centered on the screen
            screen_center_x = screen.get_width() // 2
            screen_center_y = screen.get_height() // 2
            message_rect = self.warning_message.get_rect(center=(screen_center_x, screen_center_y))
    ...
    def display_warning_message(self):
        self.warning_message = make_text('This is not a charity', 74, (255, 0, 0))  # Large red text
        self.warning_message_timer = pygame.time.get_ticks()  # Record the time when the message is displayed